...
import {
  ...
  type Experience,
  type SkillNode,
  type SkillConstellation,
  type Education,
} from "@/data/portfolio";

/* ═══════════════════════════════════════
   COSMOS-SPECIFIC TYPES & DATA
   ═══════════════════════════════════════ */
...
const PROJECTS = SHARED_PROJECTS;
...
function buildSolarProjects(projects: Project[]): SolarProjectSlot[] {
  ...
  projects.forEach((project) => {
    ...
    const idx = planetIdx.get(project.preferredPlanet as PlanetKey);
  ...
  return (
    <div className="cosmos-root" style={{ width: "100%", height: "100dvh", position: "relative", background: "#02040b", overflow: "hidden", cursor: isMobile ? "auto" : "none" }}>
      ...
      <a href="/classic" style={{
        position: "absolute", top: isMobile ? 56 : 66, right: isMobile ? 10 : 22, zIndex: 20,
        fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: ".1em",
        color: "rgba(255,255,255,.25)", textDecoration: "none",
        border: "1px solid rgba(255,255,255,.08)", padding: "3px 7px",
        pointerEvents: "auto",
        opacity: ui.loaded ? 1 : 0, transition: "opacity .8s ease .3s, color .3s ease, border-color .3s ease",
      }}
        onMouseEnter={e => { (e.target as HTMLElement).style.color = "#FF6B35"; (e.target as HTMLElement).style.borderColor = "#FF6B35"; }}
        onMouseLeave={e => { (e.target as HTMLElement).style.color = "rgba(255,255,255,.25)"; (e.target as HTMLElement).style.borderColor = "rgba(255,255,255,.08)"; }}
      >[STATIC]</a>
